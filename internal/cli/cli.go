...
func (cli *CLI) makeSpectator(sendPlayerLeft bool) {
	if !cli.isSpectator {
		...
		if spectateBtnIdx := cli.actionsForm.GetButtonIndex("spectate"); spectateBtnIdx != -1 {
...
func (cli *CLI) unmakeSpectator() {
	if cli.isSpectator {
		...
		if joinBtnIdx := cli.actionsForm.GetButtonIndex("join"); joinBtnIdx != -1 {
...
func (cli *CLI) unmakeAdmin(lock bool) {
	if cli.isTableAdmin {
		...
		if startGameBtnIdx := cli.actionsForm.GetButtonIndex("start game"); startGameBtnIdx != -1 {
		...
		if adminHeaderIdx := cli.settingsForm.GetFormItemIndex("admin options"); adminHeaderIdx != -1 {
			cli.settingsForm.RemoveFormItem(adminHeaderIdx)
		}

		if tableLockDropDownIdx := cli.settingsForm.GetFormItemIndex("table lock"); tableLockDropDownIdx != -1 {
			cli.settingsForm.RemoveFormItem(tableLockDropDownIdx)
		}

		if passwordIdx := cli.settingsForm.GetFormItemIndex("table password"); passwordIdx != -1 {
